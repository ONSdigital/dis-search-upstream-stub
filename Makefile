...
.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dis-search-upstream-stub
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dis-search-upstream-stub
...
produce: ## Runs a Kafka producer to write messages to Kafka topic from data directory JSON files
	HUMAN_LOG=1 go run kafka-tools/producer/main.go
